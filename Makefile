...
.PHONY: help test build run backend-verify backend-build backend-run frontend-install frontend-lint frontend-test frontend-build frontend-validate validate jwt-keys compose-config compose-up compose-up-detached compose-down compose-down-volumes compose-logs compose-ps smoke
...
compose-up: jwt-keys ## Build and run the full stack in the foreground
	$(COMPOSE) up --build

compose-up-detached: jwt-keys ## Build and run the full stack in the background
	$(COMPOSE) up -d --build
